...
import { registerUser } from "./actions" // Asigură-te că funcția există în acțiunile tale
    ...
    <main className="flex min-h-dvh flex-col items-center justify-center p-5 bg-zinc-950 text-zinc-100">
      ...
      <div className="w-full max-w-sm rounded-3xl border border-zinc-800/80 bg-zinc-900/60 p-8 shadow-2xl backdrop-blur-md relative overflow-hidden">
        ...
        <div className="mb-6 relative z-10">
          <h2 className="text-xl font-bold text-zinc-100">Create an account</h2>
          <p className="text-xs text-zinc-400 mt-1">Sign up to log your training</p>
        </div>
        ...
        <form action={handleAction} className="flex flex-col gap-4 relative z-10">
          ...
          >
            {isPending ? <><Loader2 className="mr-2 size-4 animate-spin" /> Creating account...</> : "Sign up"}
          </button>
        </form>
      </div>
